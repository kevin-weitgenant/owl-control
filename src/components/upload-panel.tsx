...
export const UploadPanel: React.FC<UploadPanelProps> = ({ className = '' }) => {
  ...
  const bytesPercentage = progress.totalBytes > 0
    ? Math.round((progress.bytesUploaded / progress.totalBytes) * 100)
    : 0;

  return (
    <div className={`bg-[#13151a] border border-[#2a2d35] rounded-lg p-6 ${className}`}>
      {/* Header */}
      ...
      <div className="space-y-4">
        ...
        <p className="text-xs text-gray-500 text-center">
          Upload your recorded gameplay data to contribute to the research dataset
        </p>
      </div>
    </div>
  );
