...
export class ElectronService {
   ...
   * Save credentials
   */
  public static async saveCredentials(key: string, value: string): Promise<{success: boolean; error?: string}> {
    const ipcRenderer = this.getIpcRenderer();
   ...
   * Load credentials
   */
  public static async loadCredentials(): Promise<{success: boolean; data: any; error?: string}> {
    const ipcRenderer = this.getIpcRenderer();
    if (!ipcRenderer) return {success: false, data: {}, error: 'IPC not available'};
   ...
   * Save preferences
   */
  public static async savePreferences(preferences: any): Promise<{success: boolean; error?: string}> {
    const ipcRenderer = this.getIpcRenderer();
    if (!ipcRenderer) return {success: false, error: 'IPC not available'};
   ...
   * Load preferences
   */
  public static async loadPreferences(): Promise<{success: boolean; data: any; error?: string}> {
    const ipcRenderer = this.getIpcRenderer();
    if (!ipcRenderer) return {success: false, data: {uploadFrequency: 'one', showRecordButton: true}, error: 'IPC not available'};
