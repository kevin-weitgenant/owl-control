...
def process_video(video_dir, return_tensor = False):
    ...
    button_data = button_data[button_data['timestamp'] >= start_time].reset_index(drop=True)
    ...
    if not end_rows.empty:
        ...
        button_data = button_data[button_data['timestamp'] <= end_time].reset_index(drop=True)

    # Filter for keyboard and mouse button events only
    button_data = button_data[button_data['event_type'].isin(['KEYBOARD', 'MOUSE_BUTTON'])].reset_index(drop=True)
    ...
    button_data.loc[button_data['event_type'].isin(['KEY_UP', 'MOUSE_UP']), 'event_type'] = 'UP'
    button_data.loc[button_data['event_type'].isin(['KEY_DOWN', 'MOUSE_DOWN']), 'event_type'] = 'DOWN'
    ...
    button_data = button_data.groupby(['frame', 'event_args']).apply(process_frame_events).reset_index(drop=True)
