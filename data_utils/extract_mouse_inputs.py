...
def process_video(video_dir, return_tensor = False):
    ...
    mouse_data = mouse_data[mouse_data['timestamp'] >= start_time].reset_index(drop=True)
    ...
    if not end_rows.empty:
        ...
        mouse_data = mouse_data[mouse_data['timestamp'] <= end_time].reset_index(drop=True)

    # Extract mouse movement data
    mouse_moves = mouse_data[mouse_data['event_type'] == 'MOUSE_MOVE'].reset_index(drop=True)
    ...
    # Aggregate by frame
    frame_data = mouse_moves.groupby('frame').agg({
        'dx': 'mean',
        'dy': 'mean'
    }).reset_index()
